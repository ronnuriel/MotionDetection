...
def visualize_motion(mhi, timestamp):
    ...
    vis = cv2.applyColorMap(vis, cv2.COLORMAP_JET)  # Apply color mapping
    return vis

# Streamer Process (unchanged)
...
def detector(detector_queue, presenter_queue):
    ...
    while True:
        ...
        mhi_vis = visualize_motion(motion_history, timestamp)

        presenter_queue.put((frame, mhi_vis))
...
def presenter(presenter_queue):
    ...
    while True:
        frame, mhi_vis = presenter_queue.get()
        if frame is None:
            break

        # Show both the original frame and the motion history image
        combined = np.hstack((frame, mhi_vis))

        # Display the frame with motion history visualization
